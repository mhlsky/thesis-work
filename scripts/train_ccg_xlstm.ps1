...
$configPath = Join-Path $repoRoot "configs/ccg_xlstm.yaml"
$startTime = Get-Date
$modeLabel = if ($Smoke) { "SMOKE" } else { "FULL" }

Write-Host ("=" * 80)
Write-Host "[Run Start] Stage=Train | Model=CCG-xLSTM | Mode=$modeLabel"
...
try {
    ...
    Write-Host ("=" * 80)
    Write-Host "[Run Done] Model=CCG-xLSTM | TotalElapsed=$elapsedText"
    Write-Host ("=" * 80)
}
finally {
    Pop-Location
